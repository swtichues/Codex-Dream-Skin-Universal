...
try {
  ...
  $currentCodex = Get-DreamSkinCodexInstall -AppExecutable $AppExecutable
  $codex = $currentCodex
  ...
  if ($null -ne $savedPathCandidate -and $null -eq $savedCodex -and -not $candidateMatchesCurrent) {
    ...
    if ($unverifiedSavedRunning -or $unverifiedSavedOwnsPort) {
      throw 'The saved Codex path is still active but no longer matches the selected ChatGPT/Codex desktop client. Close it manually; state was preserved.'
    }
  }
  ...
  if ($savedIsDifferent) {
    ...
    if ($currentProcesses.Count -gt 0 -and ($savedProcesses.Count -gt 0 -or $savedOwnsPort)) {
      throw 'Multiple ChatGPT/Codex desktop targets are active. Close them manually before starting Dream Skin.'
    }
    if ($savedProcesses.Count -gt 0 -or $savedOwnsPort) {
      ...
      if ($null -ne $savedIdentity) {
        ...
        Write-Warning 'Reapplying Dream Skin to the still-running saved ChatGPT/Codex target; the current Store version will be used after that app exits.'
      } else {
        $codexToStop = $savedCodex
  ...
  try {
    ...
    $state = [pscustomobject]@{
      ...
      targetKind = $codex.TargetKind
      targetDisplayName = $codex.DisplayName
      codexApplicationId = $codex.ApplicationId
      codexAppUserModelId = $codex.AppUserModelId
      codexSignatureKind = $codex.SignatureKind
  ...
  Write-Host "Codex Dream Skin is active for $($codex.DisplayName) on verified loopback port $Port."
} finally {
