try {
  $script:DreamSkinConsoleUtf8 = [System.Text.UTF8Encoding]::new($false)
  [Console]::InputEncoding = $script:DreamSkinConsoleUtf8
  [Console]::OutputEncoding = $script:DreamSkinConsoleUtf8
  $OutputEncoding = $script:DreamSkinConsoleUtf8
} catch {}
...
function Enter-DreamSkinOperationLock {
  param(
    [ValidateRange(0, 300)][int]$TimeoutSeconds = 45
  )
  ...
  if (-not $acquired) {
    $mutex.Dispose()
    $ownerDetail = ''
    $ownerPath = Join-Path $env:LOCALAPPDATA 'CodexDreamSkin\operation-owner.json'
    if (Test-Path -LiteralPath $ownerPath -PathType Leaf) {
      try {
        $owner = (Read-DreamSkinUtf8File -Path $ownerPath) | ConvertFrom-Json -ErrorAction Stop
        $ownerPid = 0
        if ([int]::TryParse("$($owner.pid)", [ref]$ownerPid) -and $ownerPid -gt 0) {
          $ownerProcess = Get-Process -Id $ownerPid -ErrorAction SilentlyContinue
          if ($ownerProcess) {
            $ownerName = if ($owner.operation) { "$($owner.operation)" } else { '未知操作' }
            $ownerDetail = " 当前操作：$ownerName，PID $ownerPid，开始时间 $($owner.startedAt)。"
          }
        }
      } catch {}
    }
    throw "另一个 Codex Dream Skin 操作仍在运行；已等待 $TimeoutSeconds 秒。$ownerDetail"
  }
  $script:DreamSkinOperationOwnerPath = Join-Path $env:LOCALAPPDATA `
    'CodexDreamSkin\operation-owner.json'
  try {
    $caller = @(Get-PSCallStack | Select-Object -Skip 1 -First 1)
    $operationName = if ($caller.Count -gt 0 -and $caller[0].ScriptName) {
      [System.IO.Path]::GetFileName("$($caller[0].ScriptName)")
    } else {
      [System.IO.Path]::GetFileName("$($MyInvocation.ScriptName)")
    }
    $processStartedAt = try {
    ...
    Write-DreamSkinUtf8FileAtomically -Path $script:DreamSkinOperationOwnerPath `
      -Content ($owner + "`r`n")
  } catch {
    # Owner metadata improves diagnostics but must never invalidate a held lock.
    $script:DreamSkinOperationOwnerPath = $null
  }
  return $mutex
}
...
function Exit-DreamSkinOperationLock {
  ...
  try {
    if ($script:DreamSkinOperationOwnerPath -and
      (Test-Path -LiteralPath $script:DreamSkinOperationOwnerPath -PathType Leaf)) {
      try {
        $owner = (Read-DreamSkinUtf8File -Path $script:DreamSkinOperationOwnerPath) |
          ConvertFrom-Json -ErrorAction Stop
        if ("$($owner.pid)" -eq "$PID") {
          Remove-Item -LiteralPath $script:DreamSkinOperationOwnerPath -Force `
...
function Stop-DreamSkinTrayProcess {
  ...
  $candidateScripts = @(
    ...
    (Get-DreamSkinRuntimeEnginePaths -StateRoot $StateRoot).Tray
  ...
  foreach ($process in $processes) {
    ...
    foreach ($trayScript in $candidateScripts) {
      if (Test-DreamSkinCommandLineToken -CommandLine "$($process.CommandLine)" -Token $trayScript) {
  ...
  if (Test-DreamSkinTrayActive) {
    throw 'Codex Dream Skin 托盘控制器仍在运行，请先从托盘菜单退出后重试。'
  }
}
...
# Load universal desktop-target compatibility overrides.
. (Join-Path $PSScriptRoot 'compat-universal.ps1')
