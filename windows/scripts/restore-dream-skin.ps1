...
try {
  ...
  $currentCodex = $null
  try { $currentCodex = Get-DreamSkinCodexInstall -AppExecutable $AppExecutable } catch { Write-Warning $_.Exception.Message }
  ...
  if ($null -ne $savedPathCandidate -and $null -eq $savedCodex -and -not $candidateMatchesCurrent) {
    ...
    if ($unverifiedSavedRunning -or $unverifiedSavedOwnsPort) {
      throw 'The saved Codex path is still active but no longer matches the selected ChatGPT/Codex desktop client. Close it manually; state and configuration were preserved.'
    }
  }
  ...
  if ($savedIsDifferent -and $currentRunning -and ($savedRunning -or $savedOwnsPort)) {
    throw 'Multiple ChatGPT/Codex desktop targets are active. Close them manually before restore; state and configuration were preserved.'
  }
  ...
  if ($savedRunning -or $savedOwnsPort -or $null -eq $currentCodex) {
    ...
    if ($null -ne $codex -and $savedIsDifferent) {
      ...
      Write-Warning 'Using the saved Codex identity after revalidating it against the selected desktop target.'
    }
  }
  ...
  $config = Get-DreamSkinConfigPath
  if ($RecoverConfigBackup) {
    ...
    if (-not (Test-Path -LiteralPath $backup)) {
      Write-Warning 'No config backup exists because config.toml was not changed during installation; only the live skin will be removed.'
